jQuery(function($) {
    ...
    function(event) {
        ...
        $.ajax({
            url: ghost.ghost_ajax + '/action/cat-ajax.php',
            data: {
                paixu: $paixu,
                cat: $cat
            },
    ...
    function() {
        ...
        $.ajax({
            url: ghost.ghost_ajax + '/action/me/' + $type + '.php',
            type: 'POST',
            success: function(msg) {
                if (msg) {
                    history.pushState(null, null, ghost.siteurl + '/me/' + $type + '/');
    ...
    function() {
        ...
        if ($msg['name'] == '' || $msg['dec'] == '') return false;
        $.ghostalert_loading(2000, false);
        $.ajax({
            url: ghost.ajaxurl,
            data: {
    ...
    function() {
        $msg = {
            'yanzhengma': $('.ghost_setting_content_preface_control[type=yanzhengma]').val(),
            'email': $('.ghost_setting_content_preface_control[type=email]').val(),
            'emailnonce': $(this).attr('data-emailnonce'),
        }
        if ($msg['yanzhengma'] == '' || $msg['email'] == '') return false;
        $.ghostalert_loading(2000, false);
        $.ajax({
            url: ghost.ajaxurl,
            data: {
    ...
    function() {
        $msg = {
            ...
            'pwdnonce': $(this).attr('data-pwdnonce'),
        }
        if ($msg['oldpwd'] == '' || $msg['newpwd'] == '' || $msg['new_pwd'] == '') return false;
        $.ghostalert_loading(2000, false);
        $.ajax({
            url: ghost.ajaxurl,
            data: {
...
// 	添加补链接
 ...
        $('body').append('<div class="ghost_addlink_bar ghost_dialog">\
        ...
        $.ajax({
            ...
            success: function(msg) {
                ...
                if (msg.status == 1) {
                    ...
                    $.ghostalert_error(msg.msg, 2000, true);
                }
            }
        });
    });
        ...
        <div class="ghost_comments_box ghost_dialog_box">\
            <div class="ghost_comments_box_container">\
                <div class="ghost_comment_fm">\
                    ...
                    <div class="ghost_comment_content">' + ghost.mypost_cat + '</div>\
                </div>\
            </div>\
        </div>\
    </div>');
                ...
                <div class="clearfix ghost_video_link">\
                    ...
                    <div class="col-lg-1 float-left poi-g_lg-1-10">\
                        <div class="poi-btn-group ghost_video_link_storage_btns">\
                            ...
                            <a style="background: rgba(241,108,102,.7);color:#fff;cursor: not-allowed;" class="ghost_video_link_delete_btn" disabled="">\
                                <span class="poi-icon fa-trash fas fa-fw" aria-hidden="true"></span>\
                            </a>\
                        </div>\
                    </div>\
                ...
                <div class="clearfix ghost_music_link">\
                    ...
                    <div class="col-lg-1 float-left poi-g_lg-1-10">\
                        <div class="poi-btn-group ghost_music_link_storage_btns">\
                            ...
                            <a style="background: rgba(241,108,102,.7);color:#fff;cursor: not-allowed;" class="ghost_music_link_delete_btn" disabled="">\
                                <span class="poi-icon fa-trash fas fa-fw" aria-hidden="true"></span>\
                            </a>\
                        </div>\
                    </div>\
    ...
    // 分类ajax
    $(".cat .ajax-morepost").click(function() {
    ...
       // notice ajax
    $('.main').on('click', '.ghost_notice_more_post .ajax-morepost',
    function() {
        $('.ajax-morepost').html("正在加载");
        $page = $('.ajax-morepost').attr('data-paged');
        $('.ajax-morepost').attr('data-paged', ++$page);
        console.log($page);
        $.ajax({
            url: ghost.ghost_ajax + "/action/me/notice_ajax.php",
            type: 'POST',
            data: {
                page: $page,
            },
    ...
    $(".my_zone .zone_type").click(function() {
        ...
        $.ajax({
            ...
            success: function(msg) {
                if (msg.status == 0) {          $('.zone_list_box').html('暂无');
                } else {
    ...
    $(".container .home_title_menu_item").click(function() {
        ...
        $this.addClass('active');
        $paixu = $this.attr('data-type');    $cat = $this.closest('.cat-container').attr('data-cat');
        if ($this.closest('.style-1').length) {
            $.ajax({
                url: ghost.ghost_ajax + "/action/style/box-ajax-1.php",
                type: 'POST',
                data: {
                    paixu: $paixu,
                    cat: $cat
            ...
            $.ajax({
                url: ghost.ghost_ajax + "/action/style/box-ajax-2.php",
                type: 'POST',
                data: {
                    paixu: $paixu,
                    cat: $cat
            ...
            $.ajax({
                url: ghost.ghost_ajax + "/action/style/box-ajax-3.php",
                type: 'POST',
                data: {
                    paixu: $paixu,
                    cat: $cat
    ...
    $('.switch_set').click(function() {
        ...
        $(this).addClass('picked');
        if (ghost.user_limit) {
            $('.ghost_play').attr('src',ghost.siteurl+'/player/?url='+$(this).attr('data-url'));
        }
    })
